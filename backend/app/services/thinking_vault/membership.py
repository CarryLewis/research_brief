...
- A non-folder page may belong to at most one folder.
- Conflicts leave the member at the default root path and emit a warning.
...
def build_folder_membership(
    ...
    *,
    ...
    folder_ids = {f.source_id for f in folders}
    ...
    for mid, folder_list in claims.items():
        ...
        member = by_id.get(mid)
        # Only thinking notes are physically placed under folder dirs.
        if member is not None and not member.is_thinking():
            if len(unique) > 1:
                result.warnings.append(
                    f"page {mid} referenced by multiple folders {unique}; "
                    "information pages stay under Information/"
                )
            continue
        if len(unique) == 1:
